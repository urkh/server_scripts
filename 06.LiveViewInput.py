...
calander_file_name = config_vars.get('calander_file_name')
        ...
        try:
            ...
            for row in contents:
                ...
                if status_val != 'Active' and status_val != 'LongRun':
                    datetime_str = str(datetime.datetime.fromtimestamp(float(time_stamp))) if not timestamp_error else time_stamp
            ...
            app_logger.log_error('Exception occured inside write_output() method. Exception message: %s' % str(msg))
...
class Main:
    ...
    def find_f2_and_f3(self,f2_row,f3_row):
        ...
        if (f2_val != 91911 and f2_val > current_time_secs) or (f3_val != 91911 and f3_val > current_time_secs):
    ...
    def find_f2_and_f3_two(self,f2_row,f3_row):
        ...
        if f2_val != 91911 or f3_val != 91911:
            ...
            if (f2_val != 0 and f2_val < current_time_secs) or (f3_val != 0 and f3_val < current_time_secs):
                ...
                while current_index != stop_index:
                    ...
                    if (f2_val != 0 and f2_val != 91911) or (f3_val != 0 and f3_val != 91911):
                        valid_data_found = True
                        break
                    current_index = self.get_previous_index(current_index)
                    f2_val = int(f2_row[current_index])
        ...
        return self.static_header[current_index],iteration_required,valid_data_found,f2_val,f3_val
    ...
    def check_and_return(self):
        ...
        if not file1_data:
            ...
            app_logger.log_error('input data is empty for file %s' % file1_name)
            return None,None
        ...
        if not file2_name:
            ...
            app_logger.log_error('input data is empty for file %s' % file2_name)
            return None,None
        ...
        if not file3_name:
            ...
            app_logger.log_error('input data is empty for file %s' % file3_name)
            return None,None

        history_file = open(history_file_name,'r')
        history_data_dict = {}
        last_line_read = 0
        for i,(last_line_read,data_seq_lines,policy_name) in enumerate(FileHandler.read_sequence_in_chunks(history_file,last_line_read)):
        ...
        def match_todays_date(timestamp):
            target_datetime = datetime.datetime.fromtimestamp(int(timestamp))
            target_date = target_datetime.date()
            tdays_date = datetime.datetime.now().date()
        ...
        def report_noschedule_data(policy_name,server_name): #datetime_val is a string representing the datetime.
        ...
        def check_valid_data_between_expectedday_today(policy_name,server_name,todays_datetime,day_gap_between_expected,policy_history_data):
            ...
            for line in policy_history_data:
                ...
                if pname == policy_name and sname == server_name:
                    ...
                    if policy_timestamp != -1:
                        policy_datetime = datetime.datetime.fromtimestamp(policy_timestamp)
            ...
            for key,value in third_col_val_dict.items():
                if key == 'SCHEDCALIDATES':
                    for index,each_tstamp in enumerate(value):
                        target_datetime = datetime.datetime.fromtimestamp(int(each_tstamp))
                    ...
                    if previous_month:
                        first_day_cm = datetime.date(day=1,month=tday_datetime.month,year=tday_datetime.year)
                        dtd = first_day_cm + timedelta(days=-1)

                    last_day = calendar.monthrange(dtd.year,dtd.month)

                    dates_in_ranges = [datetime.date(day=i + 1,month=dtd.month, year=dtd.year) for i in range(last_day[1])]
                    ...
                    previous_month = False
                    last_day_of_month = None #Last day of month. It can be current month or previous month based on decission.
                    ...
                    for index,day in enumerate(value):
                        ...
                        if previous_month:
                            ...
                            dtt = datetime.date(day=val_int,month=previous_month_date.month,year=previous_month_date.year)
                            if all_dates.get(key):
                                all_dates[key] += [dtt]
                            else:
                                all_dates[key] = [dtt]
                            ...
                            if val_int == 32:
                                last_day = calendar.monthrange(tday_datetime.year,tday_datetime.month)
                                val_int = last_day[1] # if 'val_int' is equal to 32, then we assign to this, the second value of 'last_day'
                            dtt = datetime.date(day=val_int,month=tday_datetime.month,year=tday_datetime.year)
                            if all_dates.get(key):
                                all_dates[key] += [dtt]
                            else:
                                all_dates[key] = [dtt]
        ...
        def calculate_calander_decision(policy_name,server_name,calander_date_data,file2_val,file3_val,policy_history_data):
            ...
            if third_col_val:
                ...
                if third_col_val.startswith('NotCalendar'):
                    ...
                    temp_datetime = datetime.datetime.combine(valid_data_datetime.date(),todays_datetime.time())
                    ...
                    lower_dates = []
                    for iindex,each_date in enumerate(dates_list):
        ...
        def calculate_schedule(f1_val,f2_val,f3_val):
            ...
            # Both Schedules exist in the policy - Policy well configured  and Full backup will start - OK
            if ( 0 < f2_val < 90000 and f3_val == 0 and f1_val == '0'):
                schedule = "Full"
            # Both Schedules exist in the policy - Policy well configured  and Incr backup will start - OK
            elif (f2_val == 0 and 0 < f3_val < 90000 and f1_val == '1' or f1_val == '4'):
                schedule = "Incr"
            # Both Schedules exist in the policy - Incr backup scheduled but Full backup started - WARNING
            elif (f2_val == 0 and 0 < f3_val < 90000 and f1_val == '0'):
                schedule = "MiscFull"
            # Both Schedules exist in the policy - Full backup scheduled but Incr backup started - ALERT
            elif ( 0 < f2_val < 90000 and f3_val == 0 and f1_val == '1' or f1_val == '4'):
                schedule = "IncrMisc"
            # Both Schedules exist in the policy - Policy misconfigured  both Full & Incr backup will start - WARNING
            ...
            # Both Schedules exist in the policy - Policy misconfigured but no backup will start - WARNING
            elif ( f2_val == 0 and f3_val == 0 and f1_val == '0'):
                schedule = "NoBackSched"
            # Both Schedules exist in the policy - Policy misconfigured but no backup will start - WARNING
            elif ( f2_val == 0 and f3_val == 0 and f1_val == '1' or f1_val == '4'):
                schedule = "NoBackSched"
            # No Incr schedule exist in the policy at all - Full backup only will start - OK
            elif ( 0 < f2_val < 90000 and f3_val > 90000 and f1_val == '0'):
                schedule = "FullOnly"
            # No Incr schedule exist in the policy at all - Incr backup instead of Full - ALERT
            elif ( 0 < f2_val < 90000 and f3_val > 90000 and f1_val == '1' or f1_val == '4'):
                schedule = "FullOnlyMisc"
            # No Full schedule exist in the policy at all - Incr backup only will start - ALERT
            ...
            # No Full schedule exist in the policy at all - Incr backup only will start - ALERT
                ...
                schedule = "IncrOnly"
            # No Incr schedule exist in the policy at all - Policy misconfigured but no backup will start - OK
            elif ( f2_val == 0 and f3_val > 90000 and f1_val == '0'):
                schedule = "FullSchedOnlyNoBckFull"
            # No Incr schedule exist in the policy at all - Policy misconfigured but no backup will start - WARNING (few chance to exist)
            elif ( f2_val == 0 and f3_val > 90000 and f1_val == '1' or f1_val == '4'):
                schedule = "FullSchedOnlyNoBckIncr"
            # No Full schedule exist in the policy at all - Policy misconfigured and no backup will start - WARNING
            ...
            # No Full and Incr schedule exist in the policy at all - no backup will start from this policy - WARNING
        ...
        calander_data_csv = FileHandler.read_csv(calander_file_name)
        ...
        for file1_val,file2_val,file3_val in zip(file1_data,file2_data,file3_data):
            ...
            if not data_sequence:
                ...
                if f1_val == 'Inactive':
                    ...
                    schedule = calculate_calander_decision(ppolicy_name_,pserver_name,calander_data_csv,file2_val,file3_val,data_sequence)
    ...
    def run(self):
        ...
        print '%s INFO 06.InputLiveView.py  Saving output to: %s' % (dt.strftime(dformat), output_file)
...
def run_main():
    ...
    dformat = '%Y-%m-%d %H:%M:%S'
    dt = datetime.datetime.now()
    print '%s INFO 06.InputLiveView.py  Start Generating input.txt...' % dt.strftime(dformat)
    app_logger.log_info('Start Generating input.txt...')
    dt = datetime.datetime.now()
    start_time = unix_time(dt)
    
    #print 'Start Generating input.txt...'
    #app_logger.log_info('Start Generating input.txt...')
    ...
    if not app_logger.error:
        ...
        if not column_five_percentage:
            ...
            print '%s WARNING 06.InputLiveView.py  %s must be a valid value.' % (dt.strftime(dformat), column_five_percentage)
            print '%s WARNING 06.InputLiveView.py  Program is exiting...' % dt.strftime(dformat)
            return
        try:
        ...
        if not os.path.isfile(file1_name):
            ...
            print '%s WARNING 06.InputLiveView.py  %s is not a valid file.' % (dt.strftime(dformat), file1_name)
            print '%s WARNING 06.InputLiveView.py  Program is exiting...' % dt.strftime(dformat)
            return
        if not os.path.isfile(file2_name):
            app_logger.log_warning('%s is not a valid file.' % file2_name)
            print '%s WARNING 06.InputLiveView.py  %s is not a valid file.' % (dt.strftime(dformat), file2_name)
            print '%s WARNING 06.InputLiveView.py  Program is exiting...' % dt.strftime(dformat)
            return
        if not os.path.isfile(file3_name):
            app_logger.log_warning('%s is not a valid file.' % file3_name)
            print '%s WARNING 06.InputLiveView.py  %s is not a valid file.' % (dt.strftime(dformat), file3_name)
            print '%s WARNING 06.InputLiveView.py  Program is exiting...' % dt.strftime(dformat)
            return
        if not os.path.isfile(history_file_name):
            app_logger.log_warning('%s is not a valid file' % history_file_name)
            print '%s WARNING 06.InputLiveView.py  %s is not a valid file' % (dt.strftime(dformat), history_file_name)
            print '%s WARNING 06.InputLiveView.py  Program is exiting...' % dt.strftime(dformat)
            return
        if not os.path.isfile(calander_file_name):
            app_logger.log_warning('%s is not a valid file' % calander_file_name)
            print '%s WARNING 06.InputLiveView.py  %s is not a valid file' % (dt.strftime(dformat), calander_file_name)
            print '%s WARNING 06.InputLiveView.py  Program is exiting...' % dt.strftime(dformat)
            return
        ...
        try:
            ...
            print '%s ERROR 06.InputLiveView.py  Exception occured in main block. Exception message: %s' % (td.strftime(dformat), str(msg))
            app_logger.log_error('Exception occured. Exception message: %s' % str(msg))

    
    dt = datetime.datetime.now()
    print '%s INFO 06.InputLiveView.py  noschedules reporting file: %s' % (dt.strftime(dformat), noschedult_data_file)
    app_logger.log_info('noschedules reporting file: %s' % noschedult_data_file)

    dt = datetime.datetime.now()
    print '%s INFO 06.InputLiveView.py  inactive policy reporting file: %s' % (dt.strftime(dformat), inactive_policies_data_file)
    app_logger.log_info('inactive policy reporting file: %s' % inactive_policies_data_file)

    dt = datetime.datetime.now()
    end_time = unix_time(dt)
    print '%s INFO 06.InputLiveView.py  Time to run %s seconds' % (dt.strftime(dformat), str(end_time-start_time))
    app_logger.log_info('Time to run %s seconds' % str(end_time-start_time))
    
    dt = datetime.datetime.now()
    print '%s INFO 07.InputLiveView.py  End Generating input.txt' % dt.strftime(dformat)
...
if __name__ == "__main__":
    try:
       run_main()
        ...
        dt = datetime.datetime.now()
        print '%s ERROR 06.InputLiveView.py  Script didn\'t complete successfully' % dt.strftime('%Y-%m-%d %H:%M:%S')
