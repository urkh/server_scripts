...
class MainIndex:
    ...
    def get_static_html_page_upper(self,page_title,):
        ...
        html += """\n                   <li><a href="./catalog.html" class="selected">Catalog</a></li>"""
        ...
        html += """\n        </header>"""
        html += """\n       <div id='body'></div>"""
        html += """\n           <a href='reports/catalog.csv' style="text-decoration: underline; float: right";>csv download</a></p>"""
        return html

    def generate_data_table_one(self):
        html_data_table = """"""
        html_data_table += """\n       <table class="BackupDataCatalog1">"""
        html_data_table += """\n            <tbody>"""
        html_data_table += """\n                <tr>"""
        html_data_table += """\n                    <td>Catalog</td>"""
        html_data_table += """\n                </tr>"""
        html_data_table += """\n            </tbody>"""
        html_data_table += """\n        </table>"""
        html_data_table += """\n        <table class="BackupDataCatalog2">"""
        html_data_table += """\n            <tbody>"""
        html_data_table += """\n                <tr>"""
        html_data_table += """\n                    <td class="polconfighead">Policy</td>"""
        html_data_table += """\n                    <td class="polconfighead">Backup ID</td>"""
        html_data_table += """\n                    <td class="polconfighead">Expired</td>"""
...
def run_main():
    ...
    print '%s INFO 16.Catalog2HTML.py  Output is saving to: %s' % (dt.strftime(dformat), default_html_output_file_path)
    ...
    dt = datetime.now()
    print '%s INFO 16.Catalog2HTML.py  Output Saved!' % dt.strftime(dformat)
    app_logger.log_info('Output Saved!')


if __name__ == "__main__":
    dt = datetime.now()
    print '%s INFO 16.Catalog2HTML.py  Program is starting...' % dt.strftime(dformat)
    app_logger.log_info('Program is starting...')
    print '%s INFO 16.Catalog2HTML.py  Started running...' % dt.strftime(dformat)
    ...
    try:
        ...
        dt = datetime.now()
        print '%s ERROR 16.Catalog2HTML.py  Script didn\'t complete sucessfully.' % dt.strftime(dformat)
        app_logger.log_error('Script didn\'t complete successfully.')

    dt = datetime.now()
    end_time = unix_time(dt)

    print '%s INFO 16.Catalog2HTML.py  Time to run %s seconds.' % (dt.strftime(dformat), str(end_time-start_time))
    app_logger.log_info('Time to run %s seconds.' % str(end_time-start_time))

    print '%s INFO 16.Catalog2HTML.py  Ended running...' % dt.strftime(dformat)
